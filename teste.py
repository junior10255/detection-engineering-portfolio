#!/usr/bin/env python3
"""
Detection Engineering Portfolio Organizer - v5 (Pro Edition)

Features:
- CLI (dry-run, verbose)
- Cache de metadados
- Proteção contra overwrite inteligente
- Validação robusta de YAML
...
def extrair_metadados_sigma(path, verbose=False):
    ...
    try:
        ...
        log(f"⚠️ YAML inválido: {path} -> {e}", verbose)
    except Exception as e:
        log(f"⚠️ Erro: {path} -> {e}", verbose)
...
def main():
    parser = argparse.ArgumentParser(description="Detection Portfolio Organizer")
    ...
    print("🚀 Organizer v5 iniciado")

    sigma_path = criar_pastas(base, args.verbose)
    organizar(base, sigma_path, args.dry_run, args.verbose)
    gerar_readme(base, sigma_path)

    print(f"✅ Finalizado | {len(CACHE)} arquivos analisados")
